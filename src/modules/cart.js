...
const [LOAD_CART, LOAD_CART_SUCCESS, LOAD_CART_FAILURE] =
  createRequestActionTypes("cart/LOAD_CART");

const [SAVE_CART, SAVE_CART_SUCCESS, SAVE_CART_FAILURE] =
...
export const loadCart = createAction(LOAD_CART, (user_id) => user_id);

export const saveCart = createAction(SAVE_CART, (user_id, course_ids) => ({
  user_id,
  course_ids,
}));
...
function* loadFailureSaga() {
  /// 적절한 유저 아니면 로컬 스토리지에서 user제거
  try {
...
export default handleActions(
  {
    [INSERT_COURSE]: (state, { payload: course }) => {
      ...
      const conflictingCourses = state.cart.filter((existingCourse) => {
        const existingSchedule = parseClassTimeToDict(
          existingCourse.class_time,
        );

        // 요일별로 시간 충돌 여부를 확인
        return Object.keys(newSchedule).some((day) => {
          return newSchedule[day].some(
            (hour) =>
              existingSchedule[day] && existingSchedule[day].includes(hour),
          );
        });
      });
      ...
      if (conflictingCourses.length > 0) {
        ...
        const conflictNames = conflictingCourses
          .map((course) => course.course_name)
          .join(", ");
        console.warn("시간이 겹쳐서 추가할 수 없습니다:", conflictNames);
        alert(
          `시간이 겹쳐서 추가할 수 없습니다. 겹치는 강의: ${conflictNames}`,
        );
        return state;
      }
