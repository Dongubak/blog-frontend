...
const MeetingTimeTableContainer = () => {
  ...
  useEffect(() => {
    ...
    cart.forEach((e) => {
      ...
      userData.forEach((e) => {
        Object.entries(parseClassTimeToDict(e.class_time)).forEach(
          ([key, value]) => {
            value.forEach((h) => {
  ...
  return (
    <TimeTable
      schedule={schedule}
      onDeleteCourseFromCart={onDeleteCourseFromCart}
    />
  );
};

export default MeetingTimeTableContainer;
